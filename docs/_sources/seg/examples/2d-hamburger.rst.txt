...
the `LevelSetLearn.utils.dim2.hamburger` module to create
...
`LevelSetLearn.utils.tohdf5` utility.
...
training routine. To do so, we first create a `LevelSetLearn`
...
which is provided by the `LevelSetLearn` module. This feature map computes simple
...
The `LevelSetLearn` module also provides another simple initialization routine that uses
