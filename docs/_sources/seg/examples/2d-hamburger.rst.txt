...
the `stat_learn_level_set.utils.dim2.hamburger` module to create
...
`stat_learn_level_set.utils.tohdf5` utility.
...
training routine. To do so, we first create a `stat_learn_level_set`
...
which is provided by the `stat_learn_level_set` module. This feature map computes simple
...
The `stat_learn_level_set` module also provides another simple initialization routine that uses 
