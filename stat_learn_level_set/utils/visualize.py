...
def interactive2d(
    ...
    pltimg = aximg.imshow(img, **img_kwargs)
    aximg.axis('off')

    if seg is not None:
        seg_lines = []
        for c in find_contours(seg.astype(np.float), 0.5):
            line = aximg.plot(c[:,1], c[:,0], **seg_kwargs)[0]
    ...
    for i in range(niters):
        ...
        u_line = []
        for c in find_contours(u[i], 0):
            line = aximg.plot(c[:,1], c[:,0], **u_kwargs)[0]
...
def interactive3d(
        img, u, seg=None, scores=None, axis=2
        img_kwargs=dict(cmap=plt.cm.gray, aspect=1, interpolation='bilinear'),
        u_kwargs=dict(c='b', ls='-', lw=2),
        seg_kwargs=dict(c='r', ls='-', lw=2)
    ):
    ...
    if img.ndim != 3:
        raise TypeError("`img` must be 3d.")
